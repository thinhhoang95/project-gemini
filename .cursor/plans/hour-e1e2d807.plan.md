<!-- e1e2d807-5138-4941-8454-2c22a148332f d05b67a7-f7f5-4d9e-a957-e4a22977fe68 -->
# Plan: Empirical hourly kernels from 4D segments
...
- [ ] Inspect `tvtw_indexer.json`, geojson volumes, master flights CSV, strictly_better_routes.csv, and nonorig 4D segment files to understand schemas and confirm how to derive volume entry times and routes.
- [ ] Implement core dataclasses (`Volume`, `EdgeId`, `TraversalRecord`) and a lightweight `VolumeGraph` helper in `src/gemini/propagation`.
- [ ] Implement a `TraversalExtractor` that, using `tvtw_indexer` and 4D segment data, emits `TraversalRecord`s for ORIGINAL and nonorig flights.
- [ ] Implement `HourlyKernelEstimator` to accumulate traversal counts (K1), compute raw and global kernels (K2), and apply shrinkage (K3) to produce per-edge, per-hour kernels.
- [ ] Create a CLI module to wire together data loading, traversal extraction, kernel estimation, and CSV output with sensible defaults for all file paths and hyperparameters.
- [ ] Add basic validation checks, log assumptions and edge-case handling, and verify kernel outputs on a small subset of edges for sanity.