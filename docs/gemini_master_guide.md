...
* **Level 1.** The chronological propagation with hourly kernels yields the backlog mean $m_{v,t}=E[Q_{v,t}]$ and variance $s_{v,t}^{2}= \operatorname{Var}(Q_{v,t})$. The total delay then has approximate  

  $$ 
  \mu_{\text{tot}} = \Delta\sum_{v,t} m_{v,t},\qquad 
  \sigma_{\text{tot}}^{2}= \Delta^{2}\sum_{v,t} s_{v,t}^{2},
  $$  
...
2. **Variance inflation.** Ignoring the negative covariances in the Poisson‑binomial sum slightly under‑estimates $\nu_{v,t}$. Multiplying $\nu_{v,t}$ by a factor $1.05\!-\!1.10$ yields conservative tail bounds without much extra work.  
